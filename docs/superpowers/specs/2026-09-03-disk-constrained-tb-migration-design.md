...
- `scylladb`:
  ...
  - memory: `mem_limit: 2g` (Scylla process'ga limitni cgroup'dan o'zi aniqlaydi: `command: --smp 2 --overprovisioned 1`; `--memory 1G` qat'iy tekshiruvi cgroup v2 da `available 500000000` deb start olmadi — 512M boshlang'ich limitda ishlab turdi, eski TB stop bo'lgach 2g ga ko'tarildi).
  - healthcheck: `cqlsh -e 'describe keyspaces'` (mavjud).
- `tb-pe`:
  - image: `thingsboard/tb-pe-node:3.4.1PE`.
  - `profiles: ["tb"]` — dastlab ishlamaydi.
  - memory: `mem_limit: 4g`, `mem_reservation: 2g`; `JAVA_OPTS: "-Xms1G -Xmx2560m"` (heap 2.5 GB; eski JVM kasr `-Xmx` qabul qilmaydi — butun son `2560m` ishlatiladi; qolgan ~1.5 GB — off-heap/Metaspace/OS).
...
|---------|-------|-------------|------|
| `tb-pe` | 4g | 2g | Heap `JAVA_OPTS=-Xms1G -Xmx2560m` (2.5G); eski RPM TB stop qilingandan keyin ishga tushadi — bir vaqtda ikkita TB ishlamaydi |
